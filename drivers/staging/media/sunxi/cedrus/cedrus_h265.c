	...
	struct cedrus_ctx *ctx, unsigned int index, unsigned int field)
{
	return ctx->codec.h265.mv_col_buf_addr + index *
	       ...
	       field * ctx->codec.h265.mv_col_buf_unit_size;
}
						...
						dma_addr_t dst_chroma_addr)
{
	...
	struct cedrus_h265_sram_frame_info frame_info = {
		...
		.bottom_pic_order_cnt = pic_order_cnt[1],
		.top_mv_col_buf_addr =
					     ...
					     struct cedrus_run *run)
{
	...
	for (i = 0; i < slice_params->num_active_dpb_entries; i++) {
		const struct v4l2_hevc_dpb_entry *dpb = &slice_params->dpb[i];
		...
		position = cedrus_buf->codec.hevc.position;
		...
		dst_luma_addr = cedrus_dst_buf_addr(ctx, buf_idx, 0);
		dst_chroma_addr = cedrus_dst_buf_addr(ctx, buf_idx, 1);
		mv_col_buf_addr[0] = cedrus_h265_frame_info_mv_col_buf_addr(ctx, position, 0);
		mv_col_buf_addr[1] = cedrus_h265_frame_info_mv_col_buf_addr(ctx, position, 1);
		pic_order_cnt[0] = dpb->pic_order_cnt[0];
		pic_order_cnt[1] = dpb->pic_order_cnt[1];

		cedrus_h265_frame_info_write_single(dev, position, dpb->field_pic,
						    pic_order_cnt,
						    mv_col_buf_addr,
						    dst_luma_addr,
						    dst_chroma_addr);
	...
	/* first aviliable position can be used to simplify position selection */
		...
		position = find_first_zero_bit(&used_dpbs, 20);

	output_buf = vb2_to_cedrus_buffer(vb2_buf);
	output_buf->codec.hevc.position = position;

	/* Output frame. */
	pic_order_cnt[0] = pic_order_cnt[1] = slice_params->slice_pic_order_cnt;
	mv_col_buf_addr[0] = cedrus_h265_frame_info_mv_col_buf_addr(ctx, position, 0);
	mv_col_buf_addr[1] = cedrus_h265_frame_info_mv_col_buf_addr(ctx, position, 1);
	dst_luma_addr = cedrus_dst_buf_addr(ctx, vb2_buf->index, 0);
	dst_chroma_addr = cedrus_dst_buf_addr(ctx, vb2_buf->index, 1);

	cedrus_h265_frame_info_write_single(dev, position,
					    slice_params->pic_struct != 0,
					    pic_order_cnt, mv_col_buf_addr,
					    dst_luma_addr, dst_chroma_addr);

	cedrus_write(dev, VE_DEC_H265_OUTPUT_FRAME_IDX, position);
}
...
static void cedrus_h265_ref_pic_list_write(struct cedrus_ctx *ctx,
					   struct cedrus_run *run,
					   const u8 list[],
					   ...
					   u32 sram_offset)
{
	...
	for (i = 0; i < num_ref_idx_active; i++) {
		...
		sram_array[i] = cedrus_buf->codec.hevc.position;
		if (dpb->rps == V4L2_HEVC_DPB_ENTRY_RPS_LT_CURR)
			      ...
			      struct cedrus_run *run)
{
	...
	if (!ctx->codec.h265.mv_col_buf_size) {
		 ...
		 */
		ctx->codec.h265.mv_col_buf_unit_size = width_in_ctb_luma *
			DIV_ROUND_UP(sps->pic_height_in_luma_samples, ctb_size_luma) *
			CEDRUS_H265_MV_COL_BUF_UNIT_CTB_SIZE + SZ_1K;

		ctx->codec.h265.mv_col_buf_size = 20 * 2 *
			ctx->codec.h265.mv_col_buf_unit_size;
	...
	cedrus_h265_frame_info_write_dpb(ctx, run);

	/* Reference picture list 0 (for P/B frames). */
	if (slice_params->slice_type != V4L2_HEVC_SLICE_TYPE_I) {
		cedrus_h265_ref_pic_list_write(ctx, run, slice_params->ref_idx_l0,
	...
	if (slice_params->slice_type == V4L2_HEVC_SLICE_TYPE_B) {
		cedrus_h265_ref_pic_list_write(ctx, run, slice_params->ref_idx_l1,
